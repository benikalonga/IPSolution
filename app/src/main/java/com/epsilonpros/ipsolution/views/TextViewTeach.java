...
public class TextViewTeach extends TextView {
    ...
    public TextViewTeach(Context context){
        super(context);
    ...
    public TextViewTeach(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        res = getResources();
        initAttrib(attrs, -1);
    ...
    public TextViewTeach(Context context, AttributeSet attrs, int defStyleAttr){
    ...
    public TextViewTeach(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
    ...
    private void initAttrib(AttributeSet attrs, int defStyleAttr){
        TypedArray attr = context.obtainStyledAttributes(attrs, R.styleable.TextViewTeach, defStyleAttr, 0);
        int policeInt = 0;
        try {
            policeInt = attr.getInt(attr.getIndex(R.styleable.TextViewTeach_police),1);
        } catch (Exception e) {
            e.printStackTrace();
        }
