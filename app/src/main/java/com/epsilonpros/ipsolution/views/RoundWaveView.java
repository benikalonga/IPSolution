...
public class RoundWaveView extends View {
    ...
    public RoundWaveView(Context context, @Nullable AttributeSet attrs) {
        ...
        speed = typedArray.getInt(R.styleable.RoundWaveView_speedInMillis, 300);
        useAnimation = typedArray.getBoolean(R.styleable.RoundWaveView_useAnimation, true);
        color = typedArray.getColor(R.styleable.RoundWaveView_fillColor, ContextCompat.getColor(getContext(),R.color.colorBluePrimary));
        typedArray.recycle();
        init(pointCount);
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            if (useAnimation) {
                ValueAnimator animator = ValueAnimator.ofFloat(dest).setDuration(speed);
